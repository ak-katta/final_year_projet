...
class QAState(BaseModel):
    ...
    def finish(self) -> None:
        ...
        # Only tests that actually ran belong in the pass rate. Dividing by
        # the whole plan meant a max_tests cap silently counted every
        # never-executed test as a failure.
        executed = (
            len(self.completed_test_ids)
            + len(self.failed_test_ids)
            + len(self.skipped_test_ids)
        )
        self.pass_rate = (
